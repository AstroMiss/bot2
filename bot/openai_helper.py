...
def construct_prompt(question: str, similar_chunks: List[str]) -> str:
  ...
  prom = f"Context information is below.\n" \
          "---------------------\n" \
          f"{formatted_chunks}\n" \
          "---------------------\n" \
...
class OpenAIHelper:
    ...
    async def __common_get_chat_response(self, chat_id: int, query: str, stream=False):
        ...
        try:
            ...
            promp2 = construct_prompt(query, similar_chunks)
            haha = {"role": "user", "content": promp2}

            common_args = {
                'model': self.config['model'] if not self.conversations_vision[chat_id] else self.config['vision_model'],
                'messages': self.conversations[chat_id] + [haha],
    ...
    async def __handle_function_call(self, chat_id, response, stream=False, times=0, plugins_used=()):
        ...
        # similar_chunks = retrieve_similar_chunks(function_response)
        # promp2 = construct_prompt(function_response, similar_chunks)
        # haha = {"role": "user", "content": promp2}
    ...
    async def __common_get_chat_response_vision(self, chat_id: int, content: list, stream=False):
        ...
        try:
            ...
            # haha2 = {"role": "user", "content": promp2}
